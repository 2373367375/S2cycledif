...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channel, out_channel):
        ...
        # conv 1x1
        self.conv1 = nn.Sequential(nn.Conv2d(in_channel*3, 128, kernel_size=1),
                                   nn.BatchNorm2d(128),)
                ...
                for j in range(num_branches):  # [320, 160]
                    if j > i:
                        fuse_layer.append(nn.Sequential(
                            ...
                            # nn.ConvTranspose2d(num_inchannels[j], num_inchannels[i], kernel_size=4, stride=2, padding=1, bias=False),
                            nn.BatchNorm2d(num_inchannels[i])))
                    elif j == i:
                        fuse_layer.append(None)
                    else:
...
class PAN_HighResolutionNet(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.layer1 = self._make_layer(block, 128, num_channels, num_blocks)  # [BASIC , 64, 64, 4]
                ...
                if i < self.stage2_cfg['NUM_BRANCHES']:
                    ...
                    for i in range(len(self.transition2)):
                        if self.transition2[i] is None:
                            continue
                        y1 = y_list[0] if i == 0 else self.transition2[i][0](y_list[0])
                        y2 = y_list[0] if i == 0 else self.transition2[i][1](y_list[1])
                        y = torch.add(y1, y2)
                        y = self.relu(y)
                        x_list.append(y)
            else:
...
class HS_HighResolutionNet(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        # stem net
        self.conv1 = nn.Conv2d(102, 102, kernel_size=3, stride=1, padding=1)
        self.bn1 = nn.BatchNorm2d(102)
        self.relu = nn.ReLU(inplace=True)
        ...
        num_blocks = self.stage1_cfg['NUM_BLOCKS'][0]  # 4
        self.layer1 = self._make_layer(block, 102, num_channels, num_blocks)  # [BASIC , 64, 64, 4]
        stage1_out_channel = block.expansion * num_channels
            ...
            if self.transition2[i] is not None:
                if i < self.stage2_cfg['NUM_BRANCHES']:
                    ...
                    for i in range(len(self.transition2)):
                        if self.transition2[i] is None:
                            continue
                        y1 = y_list[0] if i == 0 else self.transition2[i][1](self.transition2[i][0](y_list[0]))
                        y2 = y_list[0] if i == 0 else self.transition2[i][2](y_list[1])
                        y = torch.add(y1, y2)
                        y = self.relu(y)
                        x_list.append(y)
            else:
...
class mergeModule(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.conv1 = nn.Sequential(nn.ConvTranspose2d(384, 102, kernel_size=2, stride=2, padding=0),
                                   nn.BatchNorm2d(102),
                                   nn.ReLU(True))
        self.conv2 = nn.Sequential(nn.ConvTranspose2d(486, 102, kernel_size=2, stride=2, padding=0),
                                   nn.BatchNorm2d(102),
                                   nn.ReLU(True))

        self.blk1 = conv1x1(486, 102)
        self.conv3 = nn.Sequential(nn.Conv2d(486, 102, kernel_size=3, padding=1),
                                   nn.BatchNorm2d(102))
        self.blk2 = conv1x1(256, 102)
